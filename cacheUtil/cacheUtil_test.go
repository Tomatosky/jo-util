...
func TestSet(t *testing.T) {
	...
	t.Run("Basic Set", func(t *testing.T) {
		cache.Set("key1", 100)
		if val, found := cache.Get("key1"); !found || val != 100 {
	...
	t.Run("Overwrite Existing", func(t *testing.T) {
		cache.Set("key2", 200)
		cache.Set("key2", 300)
		if val, found := cache.Get("key2"); !found || val != 300 {
	...
	t.Run("Custom Expiration", func(t *testing.T) {
		cache.Set("key3", 400, 2*time.Second)
	...
	t.Run("Zero Value", func(t *testing.T) {
		cache.Set("key4", 0)
		if val, found := cache.Get("key4"); !found || val != 0 {
	...
	t.Run("Multiple Expiration Parameters", func(t *testing.T) {
		cache.Set("key5", 500, 1*time.Second, 2*time.Second, 3*time.Second)
	...
	t.Run("Concurrent Set", func(t *testing.T) {
		...
		if _, found := cache.Get("concurrent_key"); !found {
...
func TestSetIfAbsent(t *testing.T) {
	...
	t.Run("Key Not Exist - Set Success", func(t *testing.T) {
		success := cache.SetIfAbsent("key1", 100)
		if !success {
	...
	t.Run("Key Exists - Set Failed", func(t *testing.T) {
		cache.Set("key2", 200)
		success := cache.SetIfAbsent("key2", 300)
		if success {
			t.Error("SetIfAbsent failed: should return false when key exists")
		}
		if val, found := cache.Get("key2"); !found || val != 200 {
	...
	t.Run("Key Expired - Set Success", func(t *testing.T) {
		...
		success := cache.SetIfAbsent("key3", 500)
		if !success {
			t.Error("SetIfAbsent failed: should return true when key is expired")
		}
		if val, found := cache.Get("key3"); !found || val != 500 {
	...
	t.Run("Concurrent Set Same Key", func(t *testing.T) {
		...
		for i := 0; i < numGoroutines; i++ {
			go func() {
				defer wg.Done()
				if cache.SetIfAbsent("concurrent_key", 600) {
					successCount++
				}
			}()
		}
		...
		if val, found := cache.Get("concurrent_key"); !found || val != 600 {
	...
	t.Run("No Expiration Cache", func(t *testing.T) {
		noExpireCache := New[string, int](0)
		noExpireCache.Set("key4", 700)
		success := noExpireCache.SetIfAbsent("key4", 800)
		if success {
	...
	t.Run("Set Zero Value", func(t *testing.T) {
		success := cache.SetIfAbsent("key5", 0)
		if !success {
			t.Error("SetIfAbsent failed: should allow setting zero value")
		}
		if val, found := cache.Get("key5"); !found || val != 0 {
...
func TestGet(t *testing.T) {
	...
	t.Run("No Expiration Cache", func(t *testing.T) {
		noExpireCache := New[string, int](0)
		noExpireCache.Set("key3", 300)
		time.Sleep(1 * time.Second)
		if val, found := noExpireCache.Get("key3"); !found || val != 300 {
	...
	t.Run("Concurrent Read", func(t *testing.T) {
		...
		for i := 0; i < numReaders; i++ {
			go func() {
				defer wg.Done()
				if val, found := cache.Get("key5"); !found || val != 500 {
	...
	t.Run("Type Correctness", func(t *testing.T) {
		stringCache := New[string, string](time.Minute)
		stringCache.Set("key6", "value")
		if val, found := stringCache.Get("key6"); !found || val != "value" {
...
func TestGetWithExpiration(t *testing.T) {
	...
	t.Run("Existing Key with Expiration", func(t *testing.T) {
		cache.Set("key1", 100)
		val, exp, found := cache.GetWithExpiration("key1")
		if !found {
	...
	// 测试2: 获取不存在的键
	t.Run("Non-existent Key", func(t *testing.T) {
		_, _, found := cache.GetWithExpiration("nonexistent")
		if found {
	...
	t.Run("Expired Key", func(t *testing.T) {
		cache.Set("key2", 200)
		time.Sleep(1100 * time.Millisecond) // 等待1.1秒让key2过期
		_, _, found := cache.GetWithExpiration("key2")
		if found {
	...
	t.Run("No Expiration Cache", func(t *testing.T) {
		noExpireCache := New[string, int](0)
		noExpireCache.Set("key3", 300)
		val, exp, found := noExpireCache.GetWithExpiration("key3")
		if !found {
	...
	// 测试5: 自定义过期时间的键
	t.Run("Custom Expiration", func(t *testing.T) {
		cache.Set("key4", 400, 2*time.Second)
		_, exp, found := cache.GetWithExpiration("key4")
		if !found {
	...
	t.Run("Zero Value", func(t *testing.T) {
		cache.Set("key5", 0)
		val, _, found := cache.GetWithExpiration("key5")
		if !found {
	...
	t.Run("Concurrent Read", func(t *testing.T) {
		...
		for i := 0; i < numReaders; i++ {
			go func() {
				defer wg.Done()
				val, _, found := cache.GetWithExpiration("key6")
				if !found || val != 600 {
...
func TestDelete(t *testing.T) {
	...
	t.Run("Delete Non-existent Key", func(t *testing.T) {
		...
		cache.Delete("key2") // 应该不会panic或出错
		if _, found := cache.Get("key2"); found {
	...
	t.Run("Re-set After Delete", func(t *testing.T) {
		...
		cache.Delete("key5")
		cache.Set("key5", 600)
		if val, found := cache.Get("key5"); !found || val != 600 {
	...
	t.Run("Delete in No-Expiration Cache", func(t *testing.T) {
		noExpireCache := New[string, int](0)
		noExpireCache.Set("key6", 700)
		noExpireCache.Delete("key6")
		if _, found := noExpireCache.Get("key6"); found {
...
func TestItems(t *testing.T) {
	...
	t.Run("No Expiration Cache", func(t *testing.T) {
		noExpireCache := New[string, int](0)
		noExpireCache.Set("key5", 500)
		time.Sleep(1 * time.Second)
		items := noExpireCache.Items()
	...
	t.Run("Items With Expiration", func(t *testing.T) {
		cache.Set("key6", 600, 2*time.Second)
		items := cache.Items()
...
func TestFlush(t *testing.T) {
	...
	t.Run("Reuse After Flush", func(t *testing.T) {
		...
		// 添加新数据
		cache.Set("newKey", 400)

		// 验证新数据
		if val, found := cache.Get("newKey"); !found || val != 400 {
	...
	t.Run("Flush Cache with Expired Items", func(t *testing.T) {
		// 添加会过期的数据
		cache.Set("expiredKey", 500, 100*time.Millisecond)
		time.Sleep(150 * time.Millisecond) // 确保过期

		// 添加未过期数据
		cache.Set("validKey", 600)
	...
	t.Run("Flush No-Expiration Cache", func(t *testing.T) {
		noExpireCache := New[string, int](0)
		noExpireCache.Set("key4", 700)
		noExpireCache.Set("key5", 800)

		noExpireCache.Flush()

		if len(noExpireCache.Items()) != 0 {
	...
	t.Run("Memory Usage After Flush", func(t *testing.T) {
		...
		if cap1 := len(cache.Items()); cap1 >= before {
