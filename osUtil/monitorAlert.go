...
type Alert interface {
	Alert(name string, resourceType string, value float64, threshold float64, duration string)
}

// defaultAlert 默认报警实现
type defaultAlert struct{}

func (d *defaultAlert) Alert(name string, resourceType string, value float64, threshold float64, duration string) {
	logger.Log.Warn(fmt.Sprintf("[%s 资源报警] %s 当前值: %.2f%% 阈值: %.2f%% 持续时间: %s",
...
func (d *DingdingAlert) Alert(name string, resourceType string, value float64, threshold float64, duration string) {
	go func() {
...
func (g *GotifyAlert) Alert(name string, resourceType string, value float64, threshold float64, duration string) {
	go func() {
		postData := map[string]interface{}{
